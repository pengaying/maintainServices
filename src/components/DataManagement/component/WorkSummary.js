...
import SpareTaskSummary from '../../Dialog/DataManagement/WorkSummary/SpareTaskSummary';
import ArtificialSummary from '../../Dialog/DataManagement/WorkSummary/ArtificialSummary';
...
export default class WorkSummary extends Component {
  ...
  componentDidMount() {
    this.props.dispatch({
      type: 'dataManagement/WorkSummaryInitTable'
    })
  }
  render() {
    const columns = [
    ...
    let Tabledata = [];
    let tabledata = this.props.dataManagement.WorkSummaryData;
      ...
      <>
        ...
        {/* 列表查看 */}
        {
          this.state.ViewState ?
            <SpareTaskSummary
              DetailViewData={this.props.dataManagement.workSummaryViewData}
              index={12}
              zIndex={12}
              changeIndex={this.changeIndex}
  ...
  ClickLook = (key) => {
    if (key !== '' && key != null && key !== undefined) {
      this.props.dispatch({
        type: 'dataManagement/WorkSummaryView',
        payload: key
      })
    }
    this.setState({
