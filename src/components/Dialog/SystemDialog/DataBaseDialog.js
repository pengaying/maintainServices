...
@connect(({ systemModal, windowModal }) => ({ systemModal, windowModal }))
export default class DataBaseDialog extends Component {
                                ...
                                <div className={styles.treeNode}>
                                    <Tree showLine icon={<Icon type={this.state.unfold ? "plus-square" : "minus-square"} />} onSelect={this.onSelect}>
                                        <TreeNode title="装备数据库" key="0-0">
                                            ...
                                            <TreeNode title="备品备件管理数据库" key="0-0-2" />
                                            <TreeNode title="装备控制数据库" key="0-0-3" />
                                            <TreeNode title="自检数据库" key="0-0-4" />
                                            <TreeNode title="装备维修指导数据库" key="0-0-5" />
                                            <TreeNode title="装备站维修数据库" key="0-0-6" />
                                            <TreeNode title="装备站测试数据库" key="0-0-7" />
                                            <TreeNode title="维修方案数据库" key="0-0-8" />
                                        </TreeNode>
                                    </Tree>
                                </div>
                            </div>
    ...
    handleClick = (e) => {
        ...
        switch (e.key) {
            ...
            case 'allSearch':
                this.props.dispatch({
                    type: 'systemModal/allSearchShow'
                })
                break;
            case 'allDelete':
                this.props.dispatch({
                    type: 'systemModal/allDeleteShow'
                })
                break;
            case 'signalDelete':
                this.props.dispatch({
                    type: 'systemModal/signalDeleteShow'
                })
                break;
            default:
                break;
